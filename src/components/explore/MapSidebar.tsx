...
export function MapSidebar({ onFlyTo }: MapSidebarProps) {
    ...
    const handleCitySelect = (loc: any) => {
        ...
        if (window.innerWidth < 768) setIsOpen(false); // Close drawer on mobile after select
    };
    ...
    return (
        <>
            ...
            <button
                ...
                className="md:hidden fixed top-6 left-6 z-[1001] bg-slate-900 dark:bg-white text-white dark:text-slate-900 p-4 rounded-2xl shadow-2xl flex items-center gap-2 font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all"
            >
            ...
            {isOpen && (
                <div className="md:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2001]" onClick={() => setIsOpen(false)} />
            )}

            <div className={cn(
                "fixed z-[2002] transition-all duration-500 ease-out",
                // Mobile Styles (Bottom Sheet)
                "inset-x-0 bottom-0 top-auto translate-y-0 p-6 md:p-0",
                !isOpen && "translate-y-full md:translate-y-0",
                // Desktop Styles (Sidebar)
                "md:absolute md:top-6 md:left-6 md:bottom-auto md:w-80 md:space-y-4 md:pointer-events-auto md:inset-x-auto"
            )}>
                {/* Primary Control Island */}
                <div className="bg-white dark:bg-slate-900 md:bg-white/95 md:dark:bg-slate-900/95 md:backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-t-[2.5rem] md:rounded-[2rem] shadow-2xl p-5 md:p-4 space-y-5 md:space-y-4 max-h-[85vh] overflow-y-auto">
                    ...
                    <div className="px-2 flex items-center justify-between">
                        ...
                        <div className="hidden md:block p-1.5 bg-slate-50 dark:bg-white/5 rounded-lg">
                    ...
                    <div className="space-y-3 md:space-y-2">
                        ...
                        <div className="relative group">
                            ...
                            <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl p-4 md:p-3 flex items-center gap-3 hover:border-primary/30 transition-all focus-within:ring-2 focus-within:ring-primary/20">
                                ...
                                <input
                                    ...
                                    className="bg-transparent border-none outline-none text-sm md:text-xs font-bold w-full placeholder:text-slate-400"
                                />
                                {showRoleSuggestions ? (
                            ...
                            {showRoleSuggestions && (
                                <div className="absolute bottom-full md:top-full left-0 right-0 mb-4 md:mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50 max-h-60 md:max-h-80 overflow-y-auto">
                                        ...
                                        <div key={cat}>
                                            ...
                                            {roles.filter(r => r.toLowerCase().includes(filters.role.toLowerCase())).map(r => (
                                                <div
                                                    ...
                                                    className="px-4 py-3 md:py-2.5 text-sm md:text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-primary hover:text-white cursor-pointer transition-colors flex items-center justify-between"
                                                >
                                                    {r}
                        ...
                        <div className="relative group">
                            ...
                            <div
                                className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl p-4 md:p-3 flex items-center gap-3 cursor-pointer hover:border-primary/30 transition-all"
                                onClick={() => setShowTimeDropdown(!showTimeDropdown)}
                            >
                                <Clock className="w-4 h-4 text-slate-400" />
                                <span className="text-sm md:text-xs font-bold flex-1">
                            ...
                            {showTimeDropdown && (
                                <div className="absolute bottom-full md:top-full left-0 right-0 mb-4 md:mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50">
                        ...
                        <div className="relative group">
                            ...
                            <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl p-4 md:p-3 flex items-center gap-3 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                            ...
                            {showCityDropdown && (
                                <div className="absolute bottom-full md:top-full left-0 right-0 mb-4 md:mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50 max-h-40 md:max-h-60 overflow-y-auto">
                                    {DIRECTORY_LOCATIONS.filter(l => l.label.toLowerCase().includes(filters.specificLocation?.toLowerCase() || '')).map(loc => (
                                        <div
                                            ...
                                            className="px-4 py-3 md:py-2.5 text-sm md:text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-primary hover:text-white cursor-pointer transition-colors flex items-center gap-2"
                                        >
                    ...
                    <div className="pt-2 flex gap-3">
                        ...
                        <button
                            ...
                            className="p-4 md:p-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl hover:bg-neutral-100 dark:hover:bg-white/10 transition-all group"
                ...
                {/* Hint Box - Desktop only for less clutter on mobile */}
                <div className="hidden md:block bg-primary/25 border-2 border-primary/40 rounded-2xl p-4 shadow-xl animate-in slide-in-from-left-8 duration-700 delay-200">
